...
- I simuluated a **brute force attack** where multiple attempts were made to sign into the system with incorrect credentials. This was logged by the Wazuh SIEM as a 'Logon failure' with an event ID of 4625. The recommended steps were to verify if this user existed in the system through Active Directory, block further attempts, contact the endpoint owner, isolate the endpoint, reset credentials and escalate to L2 analyst.
